...
The official-website discovery subset is verified for public HTTP/HTTPS websites without SearchProvider or AI availability. It records robots decisions, discovers bounded sitemap metadata, fetches the homepage and configured-depth internal pages, extracts same-domain links, canonicalizes/deduplicates URLs, and ranks about, products/services, leadership, markets/partners, investor, reports, news, careers, and contact groups using English and Vietnamese tokens rather than an exact-path allowlist.

Search queries are generated deterministically from canonical name, aliases, country, domain, registration/tax identifiers, and requested sections. `ResearchQuery` and `SearchResult` persist query, provider, rank, URL, title, snippet, timestamp, source-type estimate, entity-match score, and selection reason. Snippets remain discovery metadata and are not evidence. Same-name results without strong identity signals are retained for review and are not automatically selected. The implementation does not claim TASK-CRAWL-004 parsing or full crawl behavior before the TASK-CRAWL-004 addendum below.
