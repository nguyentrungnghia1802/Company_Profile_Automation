...
The implementation intentionally stopped at discovery metadata before TASK-CRAWL-004. The verified TASK-CRAWL-004 implementation now performs bounded HTTP-first crawl/fetch, parser metadata and evidence persistence, controlled browser fallback, PDF/structured parsing, and high-precision deterministic facts without requiring AI.

## Verified implementation addendum — TASK-CRAWL-004 (2026-08-09)

`CrawlCoordinator` follows only normalized same-domain HTTP(S) links within depth, per-domain, and per-job budgets. `WebFetcher` is direct-HTTP primary and manually revalidates DNS/IP safety before every request and redirect. Timeout, redirect, response/decompression bytes, MIME, retry, rate, and per-domain concurrency limits are enforced and recorded on `SourceFetchAttempt`; browser rendering is only considered for JS-like/404 pages when enabled, policy-allowed, and within its fallback budget.

`DocumentParser` uses a deterministic malformed-HTML-tolerant parser for title, meta/OpenGraph, JSON-LD, headings, paragraphs, lists, tables, links, and section paths. Stable block keys, hashes, offsets, locations, language, and parser versions are persisted. Structured JSON blocks retain sorted field paths and provider provenance. `PDFDocumentParser` uses the pinned `pypdf` dependency, preserves page/block keys, and fails safely for oversized, encrypted, or malformed PDFs. `DeterministicFactExtractor` consumes JSON-LD/structured blocks and strongly labelled text only, links every candidate to the exact `DocumentBlock`, and does not infer broad semantic fields with regex.

## Verified implementation addendum — TASK-CRAWL-005 (2026-08-09)

AI is now an explicitly optional branch with durable reason codes for disabled configuration, missing Gemini credentials, kill-switch/provider unavailability, and timeout/provider failures. When AI is unavailable, acquisition, parsing, deterministic facts/evidence, conflict detection, and review-task creation continue; only semantic AI extraction, translation, comparison, and summary are marked skipped. Useful acquisition with a degraded optional branch is reported as `partial_success`, while an empty acquisition completes with a clear partial result and no fabricated source or fact.

The Review Inbox remains AI-independent. It creates idempotent tasks for entity ambiguity, unavailable/blocked trusted providers, strong-identifier or other deterministic conflicts, and missing mandatory high-impact fields. Research progress and source views expose the durable step state, discovery provenance, authority/entity-match rationale, fetch/policy/parser status, and last fetched time. The source API now returns these fields plus fetch-attempt, snapshot, and document-block metadata; no database migration was required.
