...
Operational dashboards and runbooks must distinguish `partial_success` caused by optional AI/provider degradation from a failed acquisition. Durable warnings use non-sensitive codes such as `AI_DISABLED`, `GEMINI_KEY_MISSING`, `AI_TIMEOUT`, `SEARCH_PROVIDER_UNAVAILABLE`, and `TRUSTED_PROVIDER_UNAVAILABLE`; provider outages create review tasks and do not justify guessed source data. Review Inbox capacity is therefore part of degraded-mode operations, while source snapshots, blocks, deterministic facts, and fetch/parser audit rows remain available for follow-up.

The automated gate intentionally uses deterministic fixtures with no live egress or credentials. Staging verification of real Gemini/Search/trusted providers and an actual Playwright browser remains an operational follow-up before enabling those integrations in production.
